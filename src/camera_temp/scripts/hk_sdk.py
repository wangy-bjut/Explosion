...
def login(ip, port, username, password):
    ...
    user_id = hk_dll.NET_DVR_Login_V30(bytes(ip), port, bytes(username), bytes(password), byref(m_strDeviceInfo))
...
def logout(user_id):
    hk_dll.NET_DVR_Logout_V30(user_id)
...
def get_temperature_max(points, source_width, source_height, user_id,channel_no):
...
def get_temperature0(user_id, channel_no = 2):
    ...
    bRet = hk_dll.NET_DVR_CaptureJPEGPicture_WithAppendData(user_id, channel_no, byref(m_strJpegWithAppenData))
    if bRet:
        # 测温数据
...
def captureJPEGPicture(user_id, lChannel, dir):
    ...
    return hk_dll.NET_DVR_CaptureJPEGPicture(user_id, lChannel, byref(jpegpara), dir)
