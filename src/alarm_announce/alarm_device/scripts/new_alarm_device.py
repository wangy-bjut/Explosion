...
import rospy
from robot_msgs.srv import alarm_srv, alarm_srvResponse  # my_package.srv是指my_package文件夹下的srv而不是MyService.srv文件
...
class Alarm_device():
    def __init__(self):
        ...
        PORT_LASER = rospy.get_param('new_alarm_device/Serial_Port', '/dev/ttyalarm')
    ...
    def set_play_volume(self):
            set_play_volume = self.__instrument.write_register(registeraddress=4, value=20,
                                                               functioncode=6)
            time.sleep(0.3)
...
def alarm_service_callback(request):
    ...
    # print((list(request.list_n)))
    # if sum(list(request.list_n)) > 0:
    #     alarm_device.combined_play(list(request.list_n))
    #     time.sleep(4)  # 需要超过组合音频时常，否则音频会被截断
